...
jQuery(document).ready(function ($) {
	...
	if ($(".wrapper > article > div").first().hasClass("wp-block-cover")) {
		...
		cover
			...
			.append(
				'<div class="scrollToBottom bounce"><a href="#"><i class="scroll-icon"></i></a></div>'
			);
		$(".wp-block-cover").on("click", ".scrollToBottom", function () {
			var header =
				$(window).outerWidth() > 481 ? $(".header-inner").outerHeight() : 0;
	...
	if ($("body:not(.search) .woo-custom-filter").length) {
		...
		theWindow.scroll(function (event) {
			...
			if (st > lastScrollTop) {
				// downscroll
				if ($(window).width() > 480) {
					$(".filter-current-parent .filter-child-cat")
						.slideUp()
		...
		$(document)
			.on("mouseenter", ".woo-custom-filter", function (event) {
		...
		$(".filter-parent-cat").on("click", "span", function (e) {
			...
			if (parent.find(".filter-child-cat").css("display") == "block") {
				$(".filter-current-parent .filter-child-cat")
					.slideToggle()
					.parent()
					.removeClass("filter-current-parent");
			} else if ($(".filter-child-cat").css("display") == "block") {
				$(".filter-current-parent .filter-child-cat").slideToggle();
				$(".filter-current-parent").removeClass("filter-current-parent");
				parent
					.addClass("filter-current-parent")
					.find(".filter-child-cat")
					.slideToggle();
			} else {
				parent
					.addClass("filter-current-parent")
		...
		var parentCategory;
		$(".woo-custom-filter").on(
			"click",
			".product-parent-selector",
			function () {
	...
	var wooUrl = "https://nurifood.ch/wp-json/wc/v3/products"; // local dev enviroment keys
	...
	var auth = basicAuth(restkeys.auth);
	var pull_page = 1;
	var jsonFlag = true;
	var postsPerPage = $(".woo-custom-filter").data("postcount")
		? $(".woo-custom-filter").data("postcount")
		: 12; // ajax get request to woo rest api
				...
				success: function success(data) {
					...
					jQuery.each(data, function (index, item) {
						if (currentQuery <= queryLength) {
							...
							var imageSrc = data[index].images[0]
								? data[index].images[0].shop_catalog
								: "https://nurifood.ch/wprs/wp-content/uploads/custom-woo-placeholder.gif";
							var productHtml = $(
								'<li class="product type-product post-' +
									_id +
									" status-publish " +
									categories +
									'has-post-thumbnail shipping-taxable purchasable product-type-simple"><a href="' +
									permalink +
									'" class="woocommerce-LoopProduct-link woocommerce-loop-product__link"><img width="315" height="315" src="' +
									imageSrc +
									'" class="attachment-woocommerce_thumbnail size-woocommerce_thumbnail" alt loading="lazy" /><h2 class="woocommerce-loop-product__title">' +
									title +
									'</h2><span class="price">' +
									price +
									"</span></a></li>"
